...
#	totalnmol=$(bc -l <<< "${nmolecule[$i]}+$totalnmol")  
	...
	do
		...
		do
			...
			python checknResidue.py test.gro
...
############################################################################################################
	...
	do
		...
		# Call checknResidue to find out how many residues have been added to the output file
		python checknResidue.py $outfile
		# Read the file 'residues.txt' this is produced by checkmResidue.py to output the number of residues in the output file
		...
		# otherwise redo with box side length increase
        		...
        		echo "Box is too small rerun with bigger box"
			side=$(($side+0.1))
		fi
